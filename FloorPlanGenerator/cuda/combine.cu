...
__global__ 
void k_createPts(int16_t *d_a, int16_t *d_b, int16_t *d_pts, const int qtd_a, const int qtd_b, const int a_offset) {
	...
	int pts_idx = ((blockIdx.z * qtd_a + blockIdx.y) * qtd_b +  b_idx) * __SIZE_PTS;
	...
	// printf("a_idx: %d,\tb_idx: %d,\tpts_idx: %d,\tblockIdx.x: %d,\tblockIdx.y: %d,\tblockIdx.z: %d,\tdblockDim.x: %d,\tthreadIdx.x: %d\n", a_idx, b_idx, pts_idx - __SIZE_A, blockIdx.x, blockIdx.y, blockIdx.z, threadIdx.x);
	// printf("a_idx: %d,\tb_idx: %d,\tpts_idx: %d,\tblockIdx.x: %d,\tblockIdx.y: %d,\tblockIdx.z: %d,\tdblockDim.x: %d,\tthreadIdx.x: %d,\tdiffX: %d,\tdiffY: %d,\ta[0]: %d,\ta[1]: %d\n", a_idx, b_idx, res_idx - __SIZE_A, blockIdx.x, blockIdx.y, blockIdx.z, threadIdx.x, diffX, diffY, a[0], a[1]);
}

void gpuHandler::createPts(const std::vector<int16_t>& a, const std::vector<int16_t>& b) {
	// const int NConn = 12;
	// const long num_a = 150;
	// const int qtd_a = a.size() / __SIZE_A;
	// const int qtd_b = b.size() / __SIZE_B;

	const long NConn = 1;
	const long qtd_a = 1; 
	const long qtd_b = 200; //minimum 12
	...
	// 	int diff = qtd_a - i;
	// 	if(diff < num_a){
	// 		k_createPts<<<grid, threads>>>(d_a, d_b, d_pts, diff, qtd_b, i);
	// 	} else {
	// 		k_createPts<<<grid, threads>>>(d_a, d_b, d_pts, num_a, qtd_b, i);
	// 	}
	// }
